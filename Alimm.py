...
def lgbTrainModel( dataFile ):
    ...
    print('误差------> ', log_loss(test['is_trade'], test['predicted_score']))# 比赛给出的误差公式
...
if __name__ == "__main__":
    ...
    print(' ----------------------------------------------------------------Model 1--------------------------------------------------')
    best_iter = lgbTrainModel( newTrainData_easy )# 模型训练--easy 数据，找到最好的迭代，讲道理，模型应该换成 其他的
    # best_iter = lgbTrainModel( newTrainData_hard )# 模型训练--hard 数据 ，找到最好的迭代，讲道理，模型应该换成 其他的
    lgbPredict( newTrainData_easy, best_iter )# 模型预测-- easy 数据
    #lgbPredict( newTrainData_hard, best_iter )# 模型预测-- hard 数据
